...
def normalize_name(s: str) -> str:
    """Normalize a column name: lowercase, strip, collapse spaces, remove surrounding quotes."""
    if s is None:
        return ""
    s = str(s)
    s = s.strip().lower()
    # collapse multiple spaces and remove odd characters spacing
    s = re.sub(r'\s+', ' ', s)
    s = s.replace('\u200b', '')  # remove zero-width if any
    return s

def build_normalized_map(df: pd.DataFrame) -> dict:
    """Return dict original_col -> normalized_col"""
    return {orig: normalize_name(orig) for orig in df.columns}

def find_capacity_column_by_type(df: pd.DataFrame, unit_type: str) -> str:
    """
    Find capacity column name (original case) based on unit_type:
    - indoor  -> look for cooling capacity & kw
    - outdoor -> look for hp / horsepower
    Uses normalized column map to be robust to spacing/case differences.
    """
    norm_map = build_normalized_map(df)
    # Priority rules
    if unit_type.lower() == "indoor":
        # prefer explicit 'cooling capacity' with 'kw'
        for orig, norm in norm_map.items():
            if "cooling capacity" in norm and "kw" in norm:
                return orig
        # fallback: any column containing 'capacity' and 'kw'
        for orig, norm in norm_map.items():
            if "capacity" in norm and "kw" in norm:
                return orig
        # fallback: any column containing 'kw'
        for orig, norm in norm_map.items():
            if "kw" in norm:
                return orig
    else:  # outdoor
        # prefer hp / horsepower words
        for orig, norm in norm_map.items():
        ...
        for orig, norm in norm_map.items():
            if "horsepower" in norm or re.search(r'\bhp\b', norm):
                return orig
        # fallback: any column containing 'capacity' and 'hp' or 'horsepower'
        for orig, norm in norm_map.items():
...
# Detect capacity column robustly using normalized names
...
if cap_col is None:
    st.warning(f"No capacity column ({cap_label_type}) found in sheet. Automatic combo will be disabled for this sheet. You can still create manual combos and export rows.")
    sizes_available = []
else:
    # convert to numeric and build sizes list
...
if wizard['combination_mode'] == "Automatic":
    if cap_col is None or not sizes_available:
        st.info("Automatic mode requires a numeric capacity column in the sheet. Switch to Manual mode or pick another sheet.")
    else:
        ...
        if st.button("Generate Combos"):
            # convert input to target in cap units
            if cap_label_type == "HP":
                ...
                if unit_input == "kW":
                    ...
                    target_cap = load_val * 0.745699872  # HP to kW
                else:  # Ton
...
if 'enriched' in st.session_state:
    ...
    for r in chosen['rows']:
        ...
        sel_row = r.copy()
        # If model exists and capacity value exists, show model choices for that capacity
    ...
    # Keep metadata columns first, then any columns from original df (preserve original column order)
    ...
    if extra_cols:
        ...
        # Ensure metadata present and then show the rest
        for c in meta_cols:
